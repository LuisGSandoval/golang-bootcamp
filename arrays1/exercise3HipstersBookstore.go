...
// HipstersBookstore testing
func HipstersBookstore() {
	// ---------------------------------------------------------
	...
	if len(os.Args) != 2 {
		fmt.Println("Please provide a book's name")
		return
	}

	searchQuery := os.Args[1:2][0]
	foundValues := false
	...
	for i, title := range bookTitles {
		...
		if i == len(bookTitles)-1 && !foundValues {
			fmt.Println("We don't have the book: ", searchQuery)
		}
	}
